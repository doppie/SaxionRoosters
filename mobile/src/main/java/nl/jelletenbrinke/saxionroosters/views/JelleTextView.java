package nl.jelletenbrinke.saxionroosters.views;
...
/**
 * Custom textview to easily attach Roboto font in XML.
 * So we no longer have to call setTypeface(Typeface tf); :)
 * USAGE: Instead of creating an XML TextView you now create a XML JelleTextView
 * You can still use normal TextView object in code :)
 */
public class JelleTextView extends TextView {

    public JelleTextView(Context context) {
        super(context);
        if (isInEditMode()) return;
        parseAttributes(null);
    }

    public JelleTextView(Context context, AttributeSet attrs) {
        super(context, attrs);
        if (isInEditMode()) return;
        parseAttributes(attrs);
    }

    public JelleTextView(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void parseAttributes(AttributeSet attrs) {
        ...
        if (attrs == null) { //Not created from xml
            ...
            TypedArray values = getContext().obtainStyledAttributes(attrs, R.styleable.JelleTextView);
            typeface = values.getInt(R.styleable.JelleTextView_typeface, Roboto.ROBOTO_REGULAR);
            values.recycle();
        }
